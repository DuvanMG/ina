    ...
    const fetchData = async () => {
      try {
        ...
        do {
          ...
          totalPages = Math.ceil(data.count / 100); // Update total pages
  ...
  const handleCardClick = async (nombre, usuarioId, fecha) => {
    ...
    if (filteredPresupuestos.length > 0) {
      ...
      filteredPresupuestos.forEach((entry) => {
        const { id, rubro, subrubro, auxiliar, item, meses_presupuesto } = entry;
      ...
      await saveDataToDB(`${currentView}_rubrosData`, {
        ...
        id: filteredPresupuestos.map((entry) => entry.idPresupuesto),
